...
import SentimentPieChart from '@renderer/components/plots/piechart/SentimentPieChart'
import RawData from '@renderer/components/plots/rawdata/RawData'
...
function Posts() {
  ...
  const [cloudData, setCloudData] = useState<{ value: string; count: number }[]>([])
  const [tableData, setTableData] = useState<{ comment: string; sentiment: string }[]>([])
      ...
      .then((jsonData) => {
        jsonData.default.forEach((comment: { comment: string; assessment: string }) => {
          ...
          setTableData((prevData) => [
            ...prevData,
            { comment: comment.comment, sentiment: comment.assessment }
          ])

          const words = comment.comment.toLowerCase().split(/\W+/)
          words.forEach((word) => {
        ...
        setCloudData(formattedCloudData)
      })
      .catch((error) => {
